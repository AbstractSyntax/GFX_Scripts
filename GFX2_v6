...
# Hostname
if ((Get-ComputerInfo).WindowsProductName -notlike "*GFX2*") { 
    Write-Host "Renaming Computer to GFX2..." -ForegroundColor Yellow
    Rename-Computer -NewName "GFX2" -Force -ErrorAction SilentlyContinue 
}
...
$sharedFolderPath = "$env:USERPROFILE\Desktop\GFX2"
if (!(Test-Path $sharedFolderPath)) { New-Item -ItemType Directory -Path $sharedFolderPath -Force | Out-Null }
if (!(Get-SmbShare -Name "GFX2" -ErrorAction SilentlyContinue)) { 
    New-SmbShare -Name "GFX2" -Path $sharedFolderPath -FullAccess "Everyone" -ErrorAction SilentlyContinue 
}
