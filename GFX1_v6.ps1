# v11 Final Hybrid - Bulletproof OSCPoint Edition
...
# --- SYSTEM CONFIGURATION (Computer Name, SMB, Shares) ---
if ((Get-ComputerInfo).WindowsProductName -notlike "*GFX1*") { Rename-Computer -NewName "GFX1" -Force }
Set-SmbServerConfiguration -EnableSMB2Protocol $true -Force
Set-Service -Name "LanmanServer", "fdPHost" -StartupType Automatic | Start-Service
...
# --- OSCPOINT BULLETPROOF INSTALLATION ---
if ($oscZip -and (Test-Path $oscZip)) {
    Write-Host "Installing OSCPoint..." -ForegroundColor Yellow
    
    # 1. Prepare permanent directory (Office add-ins hate temp folders)
    $oscDir = "C:\OSCPoint"
    ...
    if ($vstoFile) {
        # 3. FORCE TRUST THE CERTIFICATE (The Secret Sauce)
        # We add the cert to Trusted Root and Trusted Publisher so the silent install isn't blocked
        ...
        if ($cert) {
            ...
            Write-Host "Certificate trusted in Root and TrustedPublisher." -ForegroundColor Green
        }

        # 4. Run the actual VSTO Installer
        $vstoInstaller = "$env:CommonProgramFiles\microsoft shared\VSTO\10.0\VSTOInstaller.exe"
        if (Test-Path $vstoInstaller) {
            # Use /i for install and /s for silent
            Start-Process $vstoInstaller -ArgumentList "/i `"$($vstoFile.FullName)`" /s" -Wait
        }

        # 5. REGISTER FOR ALL USERS (Manual Override)
        # This makes sure PowerPoint looks for it even if the VSTO installer puts it in the wrong user hive
        $regPaths = @(
        ...
        Write-Host "OSCPoint Setup Complete." -ForegroundColor Green
    }
}
