...
# Hostname
if ((Get-ComputerInfo).WindowsProductName -notlike "*GFX4*") { 
    Write-Host "Renaming Computer to GFX4..." -ForegroundColor Yellow
    Rename-Computer -NewName "GFX4" -Force -ErrorAction SilentlyContinue 
}
...
$sharedFolderPath = "$env:USERPROFILE\Desktop\GFX4"
if (!(Test-Path $sharedFolderPath)) { New-Item -ItemType Directory -Path $sharedFolderPath -Force | Out-Null }
if (!(Get-SmbShare -Name "GFX4" -ErrorAction SilentlyContinue)) { 
    New-SmbShare -Name "GFX4" -Path $sharedFolderPath -FullAccess "Everyone" -ErrorAction SilentlyContinue 
}
